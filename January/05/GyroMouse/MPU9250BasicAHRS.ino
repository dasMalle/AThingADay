...
void mouseMoveGyro()
{
  ...
  float aY =   (float)myIMU.accelCount[1]*myIMU.aRes;
  float gX =   (float)myIMU.gyroCount[0]*myIMU.gRes;
  float gY =   (float)myIMU.gyroCount[1]*myIMU.gRes;
  float gZ =   (float)myIMU.gyroCount[2]*myIMU.gRes;
...
void GyroSetUp()
{
  ...
  if (c == 0x71) // expected sensor address
  {
    ...
    myIMU.initMPU9250(); // Initialize device
    Serial.println("MPU9250 initialized for active data mode....");
    byte d = myIMU.readByte(AK8963_ADDRESS, WHO_AM_I_AK8963); // Read the WHO_AM_I register, comm test
